...
あらかじめ画像解析AIによって、各フレームの投球結果が青色で書き込まれています。これをヒントにしてください。
以下の【ルール】に従って、フレームごとの「累計トータルスコア」を正確に読み取り、JSON形式で出力してください。

【ルール】
1. 各ゲームの行の下段には、「累計トータルスコア」が書かれています。1F〜10Fまでの10個の累計スコア数字を配列にしてください。
2．1F～10Fの２投目と、必要に応じて10Fの3投目のスコアを記録します。その確認方法は、次のとおり。
　　・1フレーム目は記録対象のフレームの累計スコアと画像解析AIが記載した１投目のスコアから計算してください。決して、記録する数値を直接見に行かないでください。
  　・2フレーム以降は記録対象のフレームの累計スコアと前のフレームまでの累計スコアの差と、画像解析AIが記載した１投目のスコアの差から計算してください。決して、記録する数値を直接見に行かないでください。
3. 最終的なトータルスコアに合わせるために、2.で計算した数値を修正しないでください。
4. 最終的なトータルスコア（"total"）は、必ず「10フレーム目の下段の累計トータルスコア」に書かれている数字を読み取ってください。一番右側のスコアは、ハンディキャップが含まれている可能性があるため、採用しないでください。
5. 複数のゲームが写っている場合は、写っているすべてのゲームのデータを配列 "games" に出力してください。
6. 日付や時間は読まなくて構いません。スコアの数字だけに集中してください。
7. Markdownの記号などは一切含めず、純粋なJSON文字列だけを出力してください。
    ...
    for img_idx, res in enumerate(st.session_state.analyzed_results):
        ...
        for local_idx, row in enumerate(res['all_games_export_data']):
            ...
            if st.toggle(f"✏️ {game_name} を手動修正", key=edit_key):
                ...
                <style>
                /* 残ピン図のボタンを少し小さく */
                [data-testid="stHorizontalBlock"] div[data-testid="stBlock"] button[key^="pin_"] {
                    padding: 1px !important;
                    margin: 1px 0 !important;
                }
                [data-testid="stHorizontalBlock"] div[data-testid="stBlock"] button[key^="pin_"] p {
                    font-size: 13px !important;
                }
                </style>
                """, unsafe_allow_html=True)
                
                st.markdown("<div class='section-header'>📌 残ピン図（タップで切り替え）</div>", unsafe_allow_html=True)
