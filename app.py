...
prompt_score = """
あなたはボウリングスコア記録員ですが、愚直に正確に画像の正しい位置の数値を読み取るプロです。
...
8. 読み取った数字が正しいことを、最後に画像と見比べて再確認してください。間違いを発見した場合は、手順1.からやり直してください。決して、その部分だけを推測や引き算で修正しないでください。
...
if st.session_state.analyzed_results is None:
    ...
    # ピン判定用の二値化画像（永山も青/緑系のインクなので青チャンネルを使用）
    b_channel = img_resized[:, :, 0]
    thresh_ink = cv2.adaptiveThreshold(b_channel, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 15, 10)

    # 2. 横線の検出とゲーム枠（ブロック）の特定
    # 永山は枠線がしっかりしているので、少し長めのカーネルで検出
    h_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (150, 1))
    h_mask = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, h_kernel)
    h_dilate = cv2.dilate(h_mask, cv2.getStructuringElement(cv2.MORPH_RECT, (20, 1)), iterations=1)
    ...
    # 近接する線をグループ化（約1G分の高さで区切る）
    blocks = []
    if h_lines_info:
        current_block = [h_lines_info[0]]
        prev_y = h_lines_info[0]['y']
        for line in h_lines_info[1:]:
            # 永山は1Gごとに独立した枠。大体100px以上離れていたら別のゲームとみなす
    ...
    games_y_coords = []
    for b in blocks:
        # 2本以上の線で構成されるブロックをゲーム枠とみなす
        if len(b) >= 2 and 100 < b[0]['y'] < target_height - 50: 
            y_min = min(l['y'] for l in b)
            y_max = max(l['y'] for l in b)
            # 枠の高さが小さすぎる（単なる太い線）場合は除外
            if y_max - y_min > 40:
                games_y_coords.append((int(y_min), int(y_max)))
                cv2.rectangle(output_img, (10, int(y_min)), (target_width-10, int(y_max)), (0, 255, 0), 2)
                cv2.putText(output_img, f"Game {len(games_y_coords)}", (20, int(y_min) + 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)

    # 3. 縦線の検出と大枠（左端・右端）の特定
    v_kernel_len = int(target_height * 0.05)
    v_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, v_kernel_len))
    v_mask = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, v_kernel)
    ...
    for cnt in v_contours:
        x, y, w, h = cv2.boundingRect(cnt)
        # 永山は縦線が短い（1Gごとの枠内で途切れる）ため、高さを甘めに設定
        if h > 30:
            v_lines_x.append(x + w / 2.0)
    v_lines_x.sort()
    ...
    left_x = target_width * 0.05 # デフォルトフォールバック
    right_x = target_width * 0.95
    
    if merged_v_lines:
        # 左端は少し内側に入る（レーン番号の線などがあるため）
        for x in merged_v_lines:
            if x > target_width * 0.05:
                left_x = x
                break
        right_x = merged_v_lines[-1]
        cv2.line(output_img, (int(left_x), 0), (int(left_x), target_height), (0, 255, 255), 2)
    ...
    # 【永山用仮設定】実測距離（要調整）
    # 基準点A(左下)から基準点B(右下トータル欄)までの距離を約192.0mmと仮定
    distance_ab_px = right_x - left_x
    mm_to_px = distance_ab_px / 192.0
    
    offset_left_mm = int(1 * mm_to_px)
    offset_right_mm = int(15 * mm_to_px) 
    
    # 永山は「スコア欄」と「ピン図」が分かれている。スコア欄を切り出す
    x1_score = int(left_x + total_w * 0.12) - offset_left_mm
    x2_score = int(right_x) - offset_right_mm
    
    x1_score = max(0, x1_score)
    x2_score = min(target_width, x2_score)
    
    for (y1, y2) in games_y_coords:
        # 永山は1Gの枠(y1〜y2)の上半分がスコア、下半分がピン図
        h_game = y2 - y1
        crop_y_top = int(y1 + h_game * 0.25) # スコア数字があるあたり
        crop_y_bottom = int(y1 + h_game * 0.55)
    ...
    for i, (y1, y2) in enumerate(games_y_coords):
        ...
        # 【永山用仮設定】実測値に基づくピン配置設定（今後調整）
        pin7_x_offset_mm = 24.0 # 基準点A(左下)から7番ピンまでのX距離
        pin1_x_offset_mm = 28.0
        pin_pitch_x_mm = (pin1_x_offset_mm - pin7_x_offset_mm) / 1.5
        
        # Y座標は下辺(base_y)を基準とし、上にマイナスする（永山のピン図は下辺に近い）
        pin7_y_offset_mm = -16.0 
        pin1_y_offset_mm = -8.0 
        pin_pitch_y_mm = (pin1_y_offset_mm - pin7_y_offset_mm) / 3.0

        frame_width_px = 15.0 * mm_to_px           # 永山はフレーム間隔がすべて均等と仮定
        frame10_pitch_px = 15.0 * mm_to_px         # 10フレーム内も均等と仮定
        
        radius_px = int(0.75 * mm_to_px)           # 判定枠
        box_size_px = 1.5 * mm_to_px
        yw = int(box_size_px)
        yh = int(box_size_px)
        ...
        # 永山用ヒストグラム解析（白抜き、黒丸、点入りの3種があるため閾値計算は一旦シンプルに）
        dyn_thresh_base = 20.0
        if game_pin_pcts:
            hist, bin_edges = np.histogram(game_pin_pcts, bins=100, range=(0, 100))
            peak1_idx = np.argmax(hist[:30]) # 白抜き丸
            peak2_idx = 30 + np.argmax(hist[30:]) # 色付き
            
            if hist[peak2_idx] > 0 and peak2_idx > peak1_idx + 10:
                dyn_thresh_base = peak1_idx + (peak2_idx - peak1_idx) * 0.4
            else:
                dyn_thresh_base = peak1_idx + 15.0
        ...
        ax1.axvline(dyn_thresh, color='#FF2D55', linestyle='dashed', linewidth=2, label=f'Threshold: {dyn_thresh:.1f}%')
        ax1.legend(loc='upper right', fontsize='small')
        ax1.set_title("Pixel Distribution & Threshold (Nagayama)", fontsize='medium')
        ax1.set_xlim(10, 60)
        fig.tight_layout()
    ...
    cv2.putText(output_img, "Nagayama Copa Bowl Mode", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 0, 255), 3, cv2.LINE_AA)
