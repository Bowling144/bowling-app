        ...
        if st.button("📸 画像からオイルデータを読み取る", use_container_width=True, type="primary"):
            try:
                ...
                creds_info = json.loads(creds_json_str, strict=False)
                    ...
                    f_res = drive_service.files().list(q="name = 'Bowling_App' and mimeType = 'application/vnd.google-apps.folder' and trashed = false", fields="files(id)").execute()                items = res.get('files', [])
