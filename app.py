...
3. 最初のゲーム数: 一番上のゲームのスコア欄の上か左の数字。「1G」や「ゲーム1」や「GAME1」と記載されていることが多い。「1」などの数値のみを "start_game_num" に出力。
4. 全体の開始時刻: "HH:MM" 形式で "start_time" に出力。見つからなければ "時刻不明" にする。
5. 全体の終了時刻: "HH:MM" 形式で "end_time" に出力。見つからなければ "時刻不明" にする。
6. レーン番号: 一番上のゲームのスコア欄付近に「L:1」や「レーン1」や「使用レーン：1」と記載されていることが多い。レーン番号（「1」などの数値のみ）を "lane" に出力。見つからなければ空文字にする。
