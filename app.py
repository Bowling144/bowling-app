...
6. レーン番号: レーン番号を数値のみで "lane" に出力してください（例: "12"）。「レーン」「LANE」「LN」などの文字の近くにある数字を探してください。ラウンドワンのスコアシートの場合、「レーン」という文字の直後、または「レーン」と「氏名」という文字の間に記載されている数字がレーン番号です。見つからなければ空文字にする。
