...
【ルール】
1. 各ゲームの行の下段にある「累計トータルスコア」の数字だけを読み取り、必ず「10個の数字の配列」にしてください。
2. 文字が薄くて読めないフレームや、数字の「1」が枠線に見える場合でも、絶対に読み飛ばさないでください。もし本当に読めない場合は、前後のスコアから推測するか「0」を入れて、必ず配列の長さを「10フレーム分（10個）」に厳守してください。
3. 残ピンの図形（丸や斜線）の数を数えてスコアに代入することは絶対にやめてください。純粋に印字されている数字のテキストだけを読んでください。
