...
3. 最初のゲーム数: 一番上のゲームのスコア欄の上か左の数字。「1G」や「ゲーム1」や「GAME1」と記載されている。また、一日のゲーム数が多いと「ページ：2/3」と記載されが２枚や３枚目なり、その時はゲーム1から始まらないため、ページ数も確認してゲーム1から始まらないことを確認すること。「1」などの数値のみを "start_game_num" に出力。
