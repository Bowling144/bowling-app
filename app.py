...
【ルール】
1. 各ゲームの行の下段には、「累計トータルスコア」が書かれています。1F〜10Fまでの10個の累計スコアの数字だけをそのまま素直に読み取って、配列にしてください。
2. 絶対に引き算などの計算や、推測は行わないでください。画像に印字されている下段の数字をそのまま転記するだけです。画像解析AIが青色で書いた1投目の数字も計算に用いないでください。
3. 最終的なトータルスコア（"total"）は、必ず「10フレーム目の下段の累計トータルスコア」に書かれている数字を読み取ってください。一番右側のスコアは、ハンディキャップが含まれている可能性があるため、採用しないでください。
4. 複数のゲームが写っている場合は、写っているすべてのゲームのデータを配列 "games" に出力してください。
5. 日付や時間は読まなくて構いません。スコアの数字だけに集中してください。
