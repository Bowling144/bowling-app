...
2. 時刻は、画像の一番上から一番下まで隅々まで確認し、見つかったすべての時刻を上から順に配列として "times" に入れてください。
   ※⚠️注意：1ゲーム目付近の時刻だけを読んで満足せず、必ず「一番下の最終ゲーム」に書かれている時刻まで絶対に省略せずに全て抽出してください。
...
3. 【⚠️最重要】各ゲームの番号（"game_num"）は、画像に印字されている実際の文字（例：「GAME 7」「GAME 8」など）をそのまま読み取ってください。絶対に1からの連番を勝手に振らないでください。
