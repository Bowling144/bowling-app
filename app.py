...
# 【重要】セッションステート（記憶）の初期化
if "raw_images_data" not in st.session_state:
    st.session_state.raw_images_data = [] # 生画像データを保存
if "analyzed_results" not in st.session_state:
    st.session_state.analyzed_results = None # 解析結果を保存
    ...
    with st.spinner("Googleドライブを探索中..."):
        try:
            ...
            if not items:
                ...
                # 取得した画像をメモリに保存し、解析結果をリセット（再解析のトリガー）
...
if st.session_state.analyzed_results is None:
    ...
    for img_idx, img_data in enumerate(st.session_state.raw_images_data):
        ...
        status_text.text(f"⚙️ 画像 {img_idx+1}/{len(st.session_state.raw_images_data)} 枚目 ({file_name}) を解析中...")
        ...
        if not success:
            st.warning(f"⚠️ {file_name}: AI読み取りに失敗しました。")
    ...
    # 全画像の解析が完了したら、記憶（セッションステート）に保存して画面をリフレッシュ
    st.session_state.analyzed_results = analyzed_results
    status_text.empty()
    st.rerun()

# =========================================================
# 📍 【ブロック 11】 結果画面・登録フォーム（最大3枚対応）
# =========================================================
if st.session_state.analyzed_results:
    ...
    with st.form("register_form"):
        ...
        selected_player = st.selectbox("これらのスコアを誰のデータとして登録しますか？", player_list)
        ...
        # 複数画像のデータをループして表示
        for res in st.session_state.analyzed_results:
            st.markdown(f"#### 📄 {res['file_name']} のデータ")
            for group_idx, game in enumerate(res['gemini_data'].get("games", [])):
                game_name = game.get('game_num', f'GAME {group_idx+1}')
                
                col1, col2 = st.columns([1, 3])
                with col1:
                    is_checked = st.checkbox(f"登録 ({game_name})", value=True, key=f"check_{global_group_idx}")
                    
                    # 万が一エラーでデータが欠落していた場合の安全処理
                    export_row = res['all_games_export_data'][group_idx] if group_idx < len(res['all_games_export_data']) else None
