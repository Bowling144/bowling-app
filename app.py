...
def analyze_copa_bowl(img, ai_meta_data):
    ...
    for i, (y1, y2) in enumerate(games_y_coords):
        ...
        # ▼ 追加（永山）：ノーミス＆10フレ1,2投目ストライクの場合、強制的に「要修正フラグ(10フレ3投目隠れ)」を立てる ▼
    ...
    cv2.putText(output_img, "Nagayama Copa Bowl Mode (Line Extract)", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 3, cv2.LINE_AA)
...
def analyze_round1(img, ai_meta_data):
    ...
    for i, (y1, y2) in enumerate(games_y_coords):
        ...
        # （ラウワン）▼ 修正：ダミーデータを入れるのは「画像に記載がない終了時刻」のみ ▼
        # 開始時刻(g_start_time)は絶対にダミーで誤魔化さず、AIの抽出結果をそのまま使用する。
        # 終了時刻(g_end_time)が不明（特に最後のゲームなど）の場合のみ、開始時刻の「+5分」をセットする。
        import datetime
        if g_end_time == "時刻不明" or not g_end_time.strip():
            if g_start_time != "時刻不明" and g_start_time.strip():
                try:
                    time_parts = g_start_time.split(":")
                    if len(time_parts) >= 2:
                        h, m = int(time_parts[0]), int(time_parts[1][:2])
                        dt = datetime.datetime(2000, 1, 1, h, m) + datetime.timedelta(minutes=5)
                        g_end_time = dt.strftime("%H:%M")
                except Exception:
                    pass
        ...
        axes_y_px = int((3.2 / 2) * mm_to_px)      # （ラウワン）判定枠の楕円Y軸半径（縦高さ3.2mmの半分）
        yw = int(1.6 * mm_to_px)                   # （ラウワン）閾値判定用のクロップ幅（1.6mmに変更）
        yh = int(3.2 * mm_to_px)                   # （ラウワン）閾値判定用のクロップ高さ（3.2mm）
        ...
        dyn_thresh = dyn_thresh_base + 1.0 + offset
...
<span style="color: white; font-size: 12px; font-weight: bold; font-family: Arial, sans-serif; text-shadow: 1px 1px 2px black, -1px -1px 2px black, 1px -1px 2px black, -1px 1px 2px black;">{rate:.1f}%</span>
                ...
                def render_06_total_monthly():
                    # --- 🎯 新機能：ダーツライブ風 トータル＆月別アワード集計（フィルター対応済） ---
                    player_full_games = []
...
if st.session_state.analyzed_results is None:
    ...
    for img_idx, img_data in enumerate(st.session_state.raw_images_data):
        ...
        # ▼ 追加（共通）：ボウリング場に応じた専用プログラムへのルーティング ▼
